...
def like_matrix_step(m, P, mem, y_n, nv):
    ...
    # build phi_L for 4 symbols, shared across particles
    def phi_for_s(s):
        # [s, mem_i...] per particle will differ in the tail; handle via broadcasting
        return s

    # Build phi (Np, 4, L) in-place: first element is symbol, tail is mem
    ...
    # For AR(1) layout, expand_phi is identity; for Matern32 (2L state), take even slots.
    # Here we assume AR(1). For 2L, interleave zeros on odd slots.
    phi = phiL  # (Np,4,L) -> used as (D=L)
    phiH = jnp.conj(phi)

    # Compute S = phi^H P phi + nv, and mu = phi^H m
